...
CREATE INDEX IF NOT EXISTS req_items_requisition_idx ON requisition_items(requisition_id);

ALTER TABLE requisition_items ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "req_items_select" ON requisition_items; CREATE POLICY "req_items_select" ON requisition_items FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "req_items_insert" ON requisition_items; CREATE POLICY "req_items_insert" ON requisition_items FOR INSERT TO authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "req_items_update" ON requisition_items; CREATE POLICY "req_items_update" ON requisition_items FOR UPDATE TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "req_items_delete" ON requisition_items; CREATE POLICY "req_items_delete" ON requisition_items FOR DELETE TO authenticated USING (true);
...
CREATE TABLE IF NOT EXISTS discharge_requests (
  ...
  pharmacy_cleared_by uuid REFERENCES auth.users(id) ON DELETE SET NULL,
  billing_cleared_by uuid REFERENCES auth.users(id) ON DELETE SET NULL,
