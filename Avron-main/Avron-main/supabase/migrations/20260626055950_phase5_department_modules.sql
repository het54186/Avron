...
CREATE INDEX IF NOT EXISTS rad_modality_idx ON radiology_requests(modality);
...
CREATE TABLE IF NOT EXISTS chemo_requests (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  req_number      text UNIQUE NOT NULL DEFAULT 'CHEMO-' || to_char(now(),'YYYYMMDD') || '-' || substr(gen_random_uuid()::text,1,6),
  patient_name    text NOT NULL,
  patient_uhid    text,
...
DROP TRIGGER IF EXISTS dr_updated_at ON drug_requests; CREATE TRIGGER dr_updated_at BEFORE UPDATE ON drug_requests FOR EACH ROW EXECUTE FUNCTION set_updated_at();
DROP TRIGGER IF EXISTS rad_updated_at ON radiology_requests; CREATE TRIGGER rad_updated_at BEFORE UPDATE ON radiology_requests FOR EACH ROW EXECUTE FUNCTION set_updated_at();
DROP TRIGGER IF EXISTS lab_updated_at ON lab_requests; CREATE TRIGGER lab_updated_at BEFORE UPDATE ON lab_requests FOR EACH ROW EXECUTE FUNCTION set_updated_at();
DROP TRIGGER IF EXISTS chemo_updated_at ON chemo_requests; CREATE TRIGGER chemo_updated_at BEFORE UPDATE ON chemo_requests FOR EACH ROW EXECUTE FUNCTION set_updated_at();
